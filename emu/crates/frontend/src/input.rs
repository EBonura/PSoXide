...
//! **Select+Start chord** that opens the XMB overlay.
//!
//! # Multi-pad policy
//!
...
//! while the XMB was open never got registered until the user
//! launched a game.
//!
//! This router:
...
//! # Chord semantics
//!
//! `Select + Start` = open / close the XMB. Matches the
//! PS1-era "player holds both shoulders" convention that shipped
//! in everything from Metal Gear Solid's pause to PSP XMB.
//!
...
/// XMB menu navigation in a single call — separate fields rather
...
pub struct InputFrame {
    ...
    /// this into `XmbInput.toggle_open`, reusing the same path
    /// Escape goes through.
    pub toggle_xmb: bool,
    ...
    /// See [`Self::xmb_up`].
    pub xmb_right: bool,
    /// Rising-edge of Cross — the XMB's "enter / confirm".
    pub xmb_confirm: bool,
    /// Rising-edge of Circle — the XMB's "back / cancel".
    pub xmb_back: bool,
...
impl InputRouter {
    ...
    pub fn poll(&mut self) -> InputFrame {
        ...
        //    the chord, so `xmb_confirm` / `xmb_back` pick up on
        ...
        let toggle_xmb = chord_active && !chord_was_active;
        ...
        //    pad sees it (no xmb_* flag fires for Select alone,
        //    since CHORD_MASK bits aren't in any xmb_* field).
        let frame = InputFrame {
            ...
            toggle_xmb,
            xmb_up: (edge & button::UP) != 0,
            xmb_down: (edge & button::DOWN) != 0,
            xmb_left: (edge & button::LEFT) != 0,
            xmb_right: (edge & button::RIGHT) != 0,
            xmb_confirm: (edge & button::CROSS) != 0,
            xmb_back: (edge & button::CIRCLE) != 0,
...
fn sample_pad(gp: &gilrs::Gamepad<'_>) -> u16 {
    ...
    // AND for XMB nav, which needs directional edges.
