...
These items are documented in full in the sections below; they are collected
here so the open questions are visible up front. None is a licensing blocker
for the *code*; all concern bundled binary assets and should be resolved
before a public or commercial release. Nothing here is guessed: where a fact
is not recorded in the repo, it is listed as missing rather than invented.

1. **Exact Pexels source URLs** for `brick-wall.jpg` and `floor.jpg` were not
   captured at download time. The Pexels License permits the use; only the
   source URLs are missing. Action: locate and record the URLs, or replace
   with newly sourced images whose URLs are recorded. See "Textures".
2. **SPU tone blobs** (`tone_sine/square/triangle/sawtooth.adpcm`) are
   non-functional placeholders generated by a command that was not recorded.
   Action: regenerate from a recorded command (and capture a per-blob
   SHA-256), or delete. See "SPU Tone Blobs".
...
- Direct renders of console firmware output. Parity tests compare
  hashes only; no firmware-derived image is stored.
