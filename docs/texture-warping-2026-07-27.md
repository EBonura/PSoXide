...
`TombRaiderSubdivisionProfile` actually decides, on real cortex_v3 rooms:
...
(`submit_tomb_raider_cached_room_{triangle,quad}` and the two
...
records. Reading `submit_tomb_raider_cached_room_quad`, E6's three parts are all
...
| per-surface GTE state reload | `load_tomb_raider_view_projection_gte` is ~17 coprocessor writes once per surface | ~2.4k/frame, 0.4% |
...
Current state: `render3d.rs` uses `tomb_raider_subdivision`, four-way splits
