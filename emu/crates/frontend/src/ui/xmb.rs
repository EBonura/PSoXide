//! XMB overlay — PS3/PSP XrossMediaBar ported from psoxide-1.
//!
...
/// A menu action the XMB emits when the user confirms an item. The
/// app layer interprets these — XMB stays stateless about the
/// emulator.
///
...
pub enum MenuAction {
    ...
    /// rescan without leaving the XMB.
    RescanLibrary,
...
/// One XMB column. `icon_name` is a short tag used by tests /
...
/// An entry passed into the XMB from the library layer — minimal
/// subset of [`psoxide_settings::LibraryEntry`] the XMB needs to
/// render an item (title + id for dispatch + region/size as the
/// right-aligned value). Kept separate so the XMB module stays
...
impl XmbState {
    ...
    /// selects an item.
    pub fn update(&mut self, input: &XmbInput) -> Option<MenuAction> {
        ...
        if input.left && self.category_index > 0 {
            ...
            // the top — matches the XMB convention. The target for
    ...
    /// Draw the XMB overlay on a middle-layer painter. `dt` drives the
    /// slide animation.
    pub fn draw(&mut self, ctx: &egui::Context, dt: f32) {
        ...
        painter.rect_filled(screen, 0.0, theme::XMB_BACKDROP);
        ...
        // Matches the PSP/PS3 XMB behaviour.
        //
        ...
        let indicator_color = theme::XMB_TEXT_DIM;
...
fn build_games_category(games: &[LibraryItem]) -> Category {
    ...
    if games.is_empty() {
        ...
        // matches PSP/PS3 UX where "Refresh" sits below the
        // scrollable section.
        items.push(MenuItem {
...
/// step, reset. Renamed from "Game" — on the PSX-style XMB, the
...
mod tests {
    ...
    fn navigation_stays_in_bounds() {
        ...
        let right = XmbInput { right: true, ..Default::default() };
        ...
        let left = XmbInput { left: true, ..Default::default() };
        for _ in 0..10 {
            s.update(&left);
        }
