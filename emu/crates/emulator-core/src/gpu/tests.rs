...
fn axis_aligned_textured_quad_draws_right_to_left_order() {
    // Tekken 3 mirrors the P2 VS portrait by submitting an axis-
...
// Crash Bandicoot intro repro: the Universal/Naughty Dog screens are
...
///   Tekken fights looked like wires over a stale photo).
...
///   scarred Gran Turismo's never-repainted menu backdrop).
...
fn wireframe_erase_restores_background_not_black() {
    ...
    // (Tekken's leaderboard scrolls its text rows). When that edge ages
