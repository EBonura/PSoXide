...
6. **XMB overlay** on `egui::Order::Middle` — dims background, slides animated category icons.
7. **HUD bar** on `egui::Order::Foreground` — always on top, above XMB.
...
rolling sample per redraw: input/XMB, guest emulation, SPU/audio, command-log
