//! CL1 standalone: the chain-load CD read matrix on a raw-SDK boot.
//!
//! The engine-hosted version of this probe (hardware-tests) exposed a
//! second silicon bug before it could run: the engine's own boot hangs
//! on real hardware in current builds. This build carries the same
//! matrix on the launcher's proven stack instead: raw psx-gpu +
//! FrameBuffer + polled pad + the vblank counter, nothing else. It
//! boots straight into the run; no menu, no engine.
//!
//! Variants, one per frame, blocking while they run:
//!   V0 CTRL   read exactly as booted (BIOS DMA/IRQ state)
//!   V1 DPCR0  DPCR zeroed first (the original quiesce state)
//!   V2 LADDR  DPCR = BIOS ladder 0x07654321 (the current quiesce)
//!   V3 HIBUF  DMA destination at the chain loader's own address
//!   V4 CDDA   CD-DA played then stopped-and-settled first
//!   V5 REPL   full replica: CD-DA + IRQ masking + ladder + high buffer
//!   V6 CTRL2  control again: does the drive still behave?
//!
//! Per variant: OK bits (prepare/start/read), DPCR readback, first
//! three words read (expected "PSOX" "STRM" then the sector count),
//! FNV-1a over the sector vs expected, channel-3 MADR after the DMA,
//! drive/controller status, reader diag snapshot. QR + on-screen table.
...
impl Probe {
    ...
    fn restart(&mut self) {
        ...
        self.qr_size = 0;
        tty::println("cd-chain-probe: cl1 begin");
    }
    ...
    fn write_binary(&self, out: &mut BinaryBuffer<'_>, crc: u32) {
        ...
        out.push_u8(2); // v2: standalone raw-SDK host
    ...
    fn draw(&self, font: &FontAtlas) {
        font.draw_text(8, 8, "CD CHAIN-LOAD MATRIX CL1 SA", (255, 232, 128));
        if !self.complete {
            for (i, variant) in Variant::ALL.iter().enumerate().take(self.next_variant) {
                ...
                font.draw_text(88, y, hex8(record.fields[2]).as_str(), (232, 236, 244));
...
fn run_variant(variant: Variant, run: u8) -> VariantRecord {
    ...
    let madr_after = unsafe { psx_io::read32(psx_io::dma::Channel::Cdrom.base()) };
    unsafe { reader.stop() };
...
fn print_record(variant: Variant, record: &VariantRecord) {
    ...
    tty::print(" w0=");
    tty::print(hex8(record.fields[2]).as_str());
    tty::print(" fnv=");
...
fn main() {
    ...
    tty::println("cd-chain-probe: booted");
    ...
    loop {
        ...
        if probe.complete && pad.is_held(button::CROSS) && !prev.is_held(button::CROSS) {
            probe.restart();
        }
        prev = pad;
