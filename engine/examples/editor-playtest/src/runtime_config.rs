...
/// Cortex tiles are smaller than TR5 room sectors, so one four-way split gives
/// the desired affine correction without paying for the 16-leaf near band.
pub(super) const ROOM_TOMB_RAIDER_SUBDIVISION_LEVELS: u8 = 1;
pub(super) const ROOM_TOMB_RAIDER_SUBDIVISION_KINDS: TombRaiderSubdivisionKindMask =
    if cfg!(feature = "tr-subdivision-ceilings") {
        TombRaiderSubdivisionKindMask::ALL
    } else {
        TombRaiderSubdivisionKindMask::FLOOR_WALL
    };
...
pub(super) fn room_surface_options(record: &LevelRoomRecord) -> WorldSurfaceOptions {
    ...
    WorldSurfaceOptions::new(WORLD_BAND, room_depth_range(record))
        .with_tomb_raider_subdivision_sector_size(subdivision_sector_size)
        .with_tomb_raider_subdivision_max_levels(ROOM_TOMB_RAIDER_SUBDIVISION_LEVELS)
        .with_tomb_raider_subdivision_kinds(ROOM_TOMB_RAIDER_SUBDIVISION_KINDS)
        .with_tomb_raider_subdivision_debug_levels(cfg!(feature = "tessellation-debug"))
