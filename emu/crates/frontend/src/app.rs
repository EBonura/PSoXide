...
pub struct AppState {
    ...
    /// state survives a quick trip back to the XMB/emulator.
...
impl AppState {
    ...
    pub fn with_config_dir(override_dir: Option<PathBuf>) -> Self {
        ...
        // Seed the XMB's Games + Examples columns from the (now
        ...
        out.xmb.sync_editor_label(out.workspace.is_editor());
        out
    }
}
...
impl AppState {
    ...
    pub fn launch_entry(&mut self, entry: &LibraryEntry) -> Result<(), String> {
        ...
        match entry.kind {
            GameKind::Exe => {
                ...
                // made EXEs launched from the XMB render blank
        ...
        // immediately when they hit Enter in the XMB — matches a real
        // PS1 where selecting a disc and pressing X fires it right up.
        // The XMB's caller (`apply_menu_action::LaunchGame`) closes
        ...
        self.xmb.sync_run_label(true);
        self.xmb.sync_editor_label(false);
    ...
    /// entries, the XMB's columns show the "No … found" placeholder
    /// instead of blowing up.
    ///
    /// Also refreshes the XMB's Games + Examples columns so the
        ...
        for e in &self.library.entries {
            ...
            match e.kind {
                ...
                GameKind::DiscBin | GameKind::DiscIso => {
                    ...
                    games.push(XmbLibraryItem {
                        id,
                        title,
                        subtitle: format_subtitle(e),
                    });
                }
                GameKind::Exe => examples.push(XmbLibraryItem {
                    id: e.id.clone(),
                    title: e.title.clone(),
                    subtitle: format_subtitle(e),
                }),
    ...
    /// Flip the disc fast-boot preference, keep the XMB label in
    ...
    pub fn toggle_fast_boot_disc(&mut self) {
        ...
        self.xmb.sync_fast_boot_label(enabled);
    ...
    pub fn open_editor_workspace(&mut self) {
        ...
        self.xmb.sync_editor_label(true);
...
/// XMB, and surfaces the stopped state via the register panel. Hitting
