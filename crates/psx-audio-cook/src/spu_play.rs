//! SPU voice playback model, for measuring what the console outputs.
//!
//! A voice steps through its decoded samples at `rate / 44100` per output
//! sample and interpolates four samples with the hardware Gaussian table
//! (nocash PSX-SPX, the same table and butterfly as PSoXide's emulator SPU).
//! The Gaussian is a gentle low-pass, so it shapes the top octave of every
//! low-rate sample; measuring after it compares encoders on the sound the
//! player hears.
...
/// The four-tap Gaussian interpolation at 8-bit phase `i`, `samples` being
/// `[oldest, older, newer, newest]`.
#[inline]
pub fn gauss(samples: [i32; 4], i: usize) -> i32 {
    let i = i & 0xFF;
    let out = GAUSS_TABLE[0xFF - i] * samples[0]
        + GAUSS_TABLE[0x1FF - i] * samples[1]
        + GAUSS_TABLE[0x100 + i] * samples[2]
        + GAUSS_TABLE[i] * samples[3];
    (out >> 15).clamp(-0x8000, 0x7FFF)
}

/// Render decoded samples through a voice at `rate` Hz to 44.1 kHz. The
/// interpolator sits between the second and third newest samples, so the
/// hardware lags two source samples; this model starts two samples in so
/// output `n` lines up with source time `n * rate / 44100`. The pitch is
/// exact rather than rounded to the 12-bit pitch register so long sounds
/// stay aligned with a reference.
pub fn play(decoded: &[i16], rate: u32) -> Vec<i16> {
    let step = rate as f64 / 44_100.0;
    let n_out = (decoded.len() as f64 / step).ceil() as usize;
    let at = |i: isize| -> i32 {
        if i < 0 || i as usize >= decoded.len() {
            0
        } else {
            decoded[i as usize] as i32
        }
    };
    (0..n_out)
        .map(|n| {
            let pos = n as f64 * step + 2.0;
            let idx = pos.floor() as isize;
            let phase = (((pos - idx as f64) * 65536.0) as u32 >> 8) as usize;
            gauss([at(idx - 3), at(idx - 2), at(idx - 1), at(idx)], phase) as i16
        })
        .collect()
}

/// Magnitude response of the Gaussian interpolator at `u` cycles per source
/// sample (RMS over the 256 interpolation phases, unity at DC).
pub fn gauss_response(u: f64) -> f64 {
    let w = 2.0 * std::f64::consts::PI * u;
    let mut acc = 0.0;
    let mut dc = 0.0;
    for i in 0..256usize {
        let phi = i as f64 / 256.0;
        let taps = [
            GAUSS_TABLE[0xFF - i],
            GAUSS_TABLE[0x1FF - i],
            GAUSS_TABLE[0x100 + i],
        ...
        for (j, &t) in taps.iter().enumerate() {
            // Tap j reads sample idx - 3 + j; the output stands for idx - 2 + phi.
            let d = j as f64 - 1.0 - phi;
            re += t as f64 * (w * d).cos();
            im += t as f64 * (w * d).sin();
            sum += t as f64;
        }
