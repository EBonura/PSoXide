//! The `psx-audio-cook` command line, for cookers that are not Rust (hk-psx,
//! voxide and pico8-psx cook in Python).
//!
//! It lives in the library so a game can build it from its hydrated
//! `.psoxide` tree, where this crate is not a member of the root workspace:
//! a two-line binary in the game's own workspace depends on this crate by
//! path and calls [`run`].
//!
//! ```text
...
fn score_cmd(source: &str, input: &str, flags: &[String]) -> ExitCode {
    ...
    let original = crate::resample::to_i16(&crate::reference_44k(&src));
...
fn rates(input: &str) -> ExitCode {
    ...
    for (r, l) in ladder.iter().zip(loss) {
        let bytes = crate::adpcm_bytes(crate::resampled_len(source.samples.len(), source.rate, *r));
        println!("{r}\t{bytes}\t{l:.2}");
    }
    ExitCode::SUCCESS
}
