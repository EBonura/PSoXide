...
- **LWL shift-inversion fix** (`71979f1`) -- our LWL used
  ...
  Corrupted every unaligned word load. a commercial title's string
...
- **All-games 100M scorecard** (`a431d55`) -- `probe_all_games_100m`
  runs every disc in `~/Downloads/discs/` to 100M steps and
...
- **D**: Crash-Bandicoot progression guard (runs 300M instructions,
  ...
  self-hash. a commercial title licensed-screen VRAM self-hash at 800M.
...
- Top-bar icon controls (Menu + wireframe + memory + VRAM + monitor-
...
(I), PAL timing (J), a commercial title heaviest-load (K). Each requires a
