...
separate host workspace from the emulator. The XMB remains the launcher shell:
...
ownership. The XMB can still be opened above the editor to return to games,
...
- `emu/crates/frontend`: owns the window, XMB, workspace switch, and emulator
  preview integration.
...
1. Add a Create category to the XMB.
