...
fn main() {
    ...
    let exe_bytes = std::fs::read(
        "/Users/ebonura/Desktop/repos/PSoXide-3/build/examples/mipsel-sony-psx/release/showcase-lights.exe",
    )
    .expect("showcase-lights.exe");
...
fn dump_ppm(bus: &Bus, frame: u64) {
    ...
    let path = format!("/tmp/showcase-lights-f{:03}.ppm", frame);
