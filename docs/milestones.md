...
- **C -- Homebrew SDK triangle.** Scaffolding complete: six SDK crates (`psx-sdk` (since removed in favor of the per-subsystem crates), `psx-hw`, `psx-rt`, `psx-io`, `psx-gpu`, `psx-gte`, `psx-pad`) plus five examples (`hello-tri`, `hello-tex`, `hello-ot`, `hello-input`, `hello-gte`). The examples build and side-load via `PSOXIDE_EXE=…`; a true end-to-end MVP test (load → draw → hash on real hardware) is still pending.
