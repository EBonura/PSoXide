...
use crate::ui::{MenuOutcome, xmb::XmbInput};
...
struct Shell {
    ...
    /// that opens the XMB, and logs connect / disconnect events
    ...
    fn window_event(
        ...
        match event {
            ...
            WindowEvent::KeyboardInput {
                ...
                // The XMB *does* honour OS-level key-repeat: holding
            ...
            WindowEvent::RedrawRequested => {
                ...
                // When the XMB is open OR currently paused, the
                ...
                input.up |= pad_frame.xmb_up;
                input.down |= pad_frame.xmb_down;
                input.left |= pad_frame.xmb_left;
                input.right |= pad_frame.xmb_right;
                input.confirm |= pad_frame.xmb_confirm;
                input.back |= pad_frame.xmb_back;

                // Escape is the "PS button" — it toggles between
                // "game running" and "game paused + menu open".
                // Intercept it here so the XMB doesn't also interpret
                ...
                if input.toggle_open {
                    ...
                    if self.state.running {
                        ...
                        self.state.xmb.sync_run_label(false);
                        self.state.xmb.open = true;
                    } else if self.state.xmb.open {
                        // Menu mode → game mode: resume if we have a
                        // live game to resume; otherwise just close
                        // the overlay.
                        self.state.xmb.open = false;
                        if self.state.bus.is_some() && self.state.current_game.is_some() {
                            self.state.running = true;
                            self.state.xmb.sync_run_label(true);
                        }
                    } else {
                        // No game running and XMB already closed —
